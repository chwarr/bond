...
/// @brief A synchronization primitive that is signaled when a certian number
/// of threads are waiting on it at the same time.
class barrier final
{
public:
    /// @param count the number of threads that must enter before they will all
    /// be released. Must be positive.
    ...
    /// @brief Enter the barrier, decrement the count, and potentially release
